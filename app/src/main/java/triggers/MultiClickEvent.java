...
public class MultiClickEvent {
    ...
    public void registerClick(Long eventTime) {
        if (waitForConfirmation) {
            ...
            int vibrationDuration = 2000;

            boolean isConfirmationClickedBeforeVibrationEnded = confirmationDuration <=
                    vibrationDuration;
            boolean isConfirmationClickedWithinTimeLimit = (vibrationDuration +
        ...
        if (isFirstClick() || notWithinLimit(eventTime) || !isPowerClickBecauseOfUser()) {
            Log.e("MultiClick", "isFirstClick() || notWithinLimit(eventTime) || " +
        ...
        try {
            ...
            if (!events.isEmpty()) {
                ...
                try {
                    ...
                    if (powerEventLogData.length > 2 && (Integer) powerEventLogData[0] ==
                            IS_POWER_STATE_ASLEEP) {
                        boolean isPowerChangeUserTriggered = (Integer) powerEventLogData[1] ==
                                IS_STATE_CHANGE_USER_TRIGERRED;
                        Log.e("PowerChangeUser", "" + isPowerChangeUserTriggered);
